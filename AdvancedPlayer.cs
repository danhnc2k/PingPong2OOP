...
namespace Pong
{
    public class AdvancedPlayer
    {
        ...
        public AdvancedPlayer(PictureBox aPaddle1, Label scoreLabel)
        {
        ...
        private void DoMove(bool? goingUp)
        {
            if (goingUp.HasValue)
            {
                ...
                paddle.Location = new Point(paddle.Location.X + speed, Math.Max(PongWorldInfo.leftOfWorld, Math.Min(PongWorldInfo.rightOfWorld - paddle.Height, paddle.Location.Y )
                        )
                    );
            }
        }
